...
def Weighted_MAE_torch(pred, real, null_val=0.0, weights=None, eps=1e-8):
    """
    pred, real: shape [B, 1, N, T]  (你的代码就是这个形状)
    weights: shape [T]，horizon 权重，越大表示越重视该步
    """
    # mask：过滤 null_val
    ...
    abs_err = torch.abs(pred - real)

    # 加权的“有效平均”
    numerator = torch.sum(abs_err * mask * w)
    denom = torch.sum(mask * w) + eps
...
def load_dataset(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    for category in ["train", "val", "test"]:
        ...
        print("正在加载文件：", os.path.join(dataset_dir, category + ".npz"))
